...
export const QuickConsultationMode: React.FC<QuickConsultationModeProps> = ({ onBack }) => {
  ...
  const consultationCategories = [
    {
      ...
      description: 'High-level business strategy and direction',
    },
    {
      id: 'operational',
    ...
    },
    {
      ...
      description: 'Investment, funding, and financial planning',
    },
    {
      id: 'marketing',
    ...
    },
    {
      ...
      description: 'Product development and feature prioritization',
    },
    {
      id: 'team',
      name: 'Team & Leadership',
      description: 'Hiring, management, and organizational issues',
    },
  ];
      ...
      if (isConfigured) {
        const advisorResponses = await Promise.all(
          selectedAdvisorObjects.map(async advisor => {
            ...
            return {
              advisor: advisor?.name,
              response: generateAdvisorSpecificResponses([advisor!], selectedCategory, question)[0]
                .response,
            };
          })
        );
        ...
        const aiConsultation = {
          ...
          recommendation: generateMultiAdvisorRecommendation(
            selectedCategory,
        ...
        const mockConsultation = {
          ...
          risks: generateRisks(selectedCategory),
          followUpQuestions: generateFollowUpQuestions(selectedCategory),
          advisorResponses: generateAdvisorSpecificResponses(
            selectedAdvisorObjects,
            selectedCategory,
            question
          ),
  ...
  const generateMultiAdvisorRecommendation = (category: string, advisors: any[]) => {
    ...
    const advisorPerspectives = advisors
      .map(advisor => {
        const perspectives = {
  ...
  const generateAdvisorSpecificResponses = (
    advisors: any[],
    category: string,
    question: string
  ) => {
    return advisors.map(advisor => {
      const responses = {
        'mark-cuban': {
          strategic:
            'Before making any strategic moves, show me the data. What are your customers telling you? What do the numbers say about market demand?',
          operational:
            'Operations are about efficiency and results. What processes are slowing you down? Where are you wasting money or time?',
          financial:
            "Cash flow is everything. I need to see your burn rate, runway, and path to profitability. Don't spend money you don't have.",
          marketing:
            "Marketing without measurement is just spending money. What's your customer acquisition cost? What's working and what isn't?",
          product:
            'Build what customers actually want, not what you think they want. Have you validated this with real users?',
          team: 'Your team is your biggest asset or liability. Are you hiring A-players? Are you setting clear expectations?',
        },
        'reid-hoffman': {
          strategic:
            "Think about the ecosystem you're building. How does this decision create network effects and platform value?",
          operational:
            'Scale requires systems that work without you. How can you build operational leverage into everything you do?',
          financial:
            'Consider the long-term implications. How does this financial decision support your path to becoming a category-defining company?',
          marketing:
            "Focus on building something people love so much they tell others. What's your organic growth strategy?",
          product:
            'Product strategy should create increasing returns to scale. How does each feature make your product more valuable?',
        ...
        'barbara-corcoran': {
          strategic:
            "Strategy without execution is just a dream. Who's going to make this happen and how will you hold them accountable?",
          operational:
            "I've built businesses by focusing on what matters most. What are the 3 most important things you need to get right?",
          financial:
            "Money follows results. Instead of just looking at the numbers, tell me how you're going to improve them.",
          marketing:
            'Sales is about relationships and trust. How are you building genuine connections with your customers?',
          product:
            'Your product needs to solve a real problem people will pay for. Have you talked to enough customers to know this is true?',
      ...
      const response = advisorResponses
        ? advisorResponses[category as keyof typeof advisorResponses]
        : "That's a great question. Let me share my perspective on this important decision.";
          ...
          <div className="bg-white rounded-2xl p-8 shadow-lg">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">
                Expert advice from {consultation.advisorCount} advisor
                {consultation.advisorCount > 1 ? 's' : ''}: {consultation.advisors}
              </p>
            ...
            <div className="mb-8">
              ...
              <div className="space-y-4">
                {consultation.advisorResponses.map((response: any, index: number) => (
                  <div
                    key={index}
                    className="p-4 bg-white rounded-lg border-l-4 border-orange-500 shadow-sm"
                  >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-red-50 p-6">
      <div className="max-w-4xl mx-auto">
        <button onClick={onBack} className="mb-6 text-orange-600 hover:text-orange-700 font-medium">
          ← Back to Dashboard
        </button>
        ...
        <div className="bg-white rounded-2xl p-8 shadow-lg">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">
              Get instant expert advice for immediate business decisions
            </p>
            {isConfigured ? (
              <div className="inline-flex items-center mt-2 px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm">
                <span className="w-2 h-2 bg-green-500 rounded-full mr-2"></span>
          ...
          <div className="mb-8">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">
              Choose Your Advisory Panel ({selectedAdvisors.length} selected)
            </h2>
            <p className="text-sm text-gray-600 mb-4">
              Select multiple advisors to get diverse perspectives on your decision
            </p>
