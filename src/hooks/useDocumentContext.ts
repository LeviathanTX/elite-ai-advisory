...
export const useDocumentContext = () => {
  ...
  const parseDocumentReferences = useCallback(
    (messageText: string, advisorDocuments: StoredDocument[]): DocumentReference[] => {
  ...
  const suggestRelevantDocuments = useCallback(
    (
      ...
      return documentContextService.suggestRelevantDocuments(
        conversationText,
        advisorDocuments,
        limit
      );
  ...
  const getDocumentById = useCallback(
    (documentId: string): StoredDocument | undefined => {
  ...
  const getDocumentPreview = useCallback(
    (documentId: string, maxLength = 300): string => {
